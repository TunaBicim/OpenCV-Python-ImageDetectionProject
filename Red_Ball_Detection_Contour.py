def Red_Ball_Detection(image):
    ...
    lower_red_down = np.array([0,60,60])
    upper_red_down = np.array([10,255,255]) 
    lower_red_up = np.array([170,60,60])
    upper_red_up = np.array([180,255,255])
    # Convert BGR to HSV
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    # Threshold the HSV image to get only blue colors
    mask1 = cv2.inRange(hsv, lower_red_down, upper_red_down)
    mask2 = cv2.inRange(hsv, lower_red_up, upper_red_up)
    # Bitwise-AND mask and original image Bitwise-OR the masked images
    residual = cv2.bitwise_and(image,image, mask= mask1)
    residual2 = cv2.bitwise_and(image,image, mask= mask2)
    cv2.bitwise_or(residual,residual2,residual)
    # Contour Detection
